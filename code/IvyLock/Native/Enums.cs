...
{
	/// <summary>
	/// Windows Messages Defined in winuser.h from Windows SDK v6.1
	...
	public enum WM : uint
	{
		...
		/// are destroyed. During the processing of the message, it can
		...
		/// use the same input queue, the message is sent
		/// synchronously, first to the window procedure of the
		/// top-level window being deactivated, then to the window
		/// procedure of the top-level window being activated. If the
		/// windows use different input queues, the message is sent
		...
		/// determine the length, in characters, of the text associated
		/// with a window.
		/// </summary>
		...
		/// <summary>
		/// The WM_QUERYENDSESSION message is sent when the user
		/// chooses to end the session or when an application calls one
		/// of the system shutdown functions. If any application
		/// returns zero, the session is not ended. The system stops
		/// sending WM_QUERYENDSESSION messages as soon as one
		/// application returns zero. After processing this message,
		/// the system sends the WM_ENDSESSION message with the wParam
		...
		/// <summary>
		/// An application sends the WM_WININICHANGE message to all
		/// top-level windows after making a change to the WIN.INI
		/// file. The SystemParametersInfo function sends this message
		/// after an application uses the function to change a setting
		/// in WIN.INI. Note The WM_WININICHANGE message is provided
		/// only for compatibility with earlier versions of the system.
		...
		/// top-level windows in the system after changing the pool of
		/// font resources.
		/// </summary>
		FONTCHANGE = 0x001D,
		...
		/// explicitly to the specified window regardless of whether it
		/// is the active window. For example, the EnableWindow
		...
		/// an inactive window and the user presses a mouse button. The
		/// parent window receives this message only if the child
		...
		/// the user clicks the window's title bar or when the window
		...
		/// from other messages sent through the WH_JOURNALPLAYBACK
		/// Hook procedure.
		/// </summary>
		QUEUESYNC = 0x0023,
		...
		/// <summary>
		/// Windows NT 3.51 and earlier: The WM_PAINTICON message is
		/// sent to a minimized window when the icon is to be painted.
		/// This message is not sent by newer versions of Microsoft
		...
		/// whenever a job is added to or removed from the Print
		/// Manager queue.
		/// </summary>
		SPOOLERSTATUS = 0x002A,

		/// <summary>
		/// The WM_DRAWITEM message is sent to the parent window of an
		...
		/// combo box, list box, list view control, or menu item when
		...
		/// box or combo box is destroyed or when items are removed by
		/// the LB_DELETESTRING, LB_RESETCONTENT, CB_DELETESTRING, or
		/// CB_RESETCONTENT message. The system sends a WM_DELETEITEM
		/// message for each deleted item. The system sends the
		/// WM_DELETEITEM message for any deleted list box or combo box
		/// item with nonzero item data.
		/// </summary>
		DELETEITEM = 0x002D,

		/// <summary>
		/// Sent by a list box with the LBS_WANTKEYBOARDINPUT style to
		...
		/// retrieve the font with which the control is currently
		/// drawing its text.
		/// </summary>
		GETFONT = 0x0031,
		...
		/// system displays this cursor or icon while the user drags
		/// the icon.
		/// </summary>
		...
		/// the relative position of a new item in the sorted list of
		/// an owner-drawn combo box or list box. Whenever the
		...
		/// <summary>
		/// Active Accessibility sends the WM_GETOBJECT message to
		/// obtain information about an accessible object contained in
		/// a server application. Applications never send this message
		/// directly. It is sent only by Active Accessibility in
		/// response to calls to AccessibleObjectFromPoint,
		/// AccessibleObjectFromEvent, or AccessibleObjectFromWindow.
		...
		/// <summary>
		/// Notifies applications that the system, typically a
		/// battery-powered personal computer, is about to enter a
		/// suspended mode.
		/// Use: POWERBROADCAST
		/// </summary>
		[Obsolete]
		...
		/// when a user cancels the application's journaling
		...
		/// passing the message to the DefWindowProc function or reject
		...
		/// settings and pass the message to the DefWindowProc
		/// function, which passes the message to all first-level child
		/// windows. These child windows can pass the message to
		/// DefWindowProc to have it pass the message to their child
		/// windows, and so on.
		/// </summary>
		...
		/// application when the user clicks an authorable button. An
		...
		/// associated with the menu; otherwise, WM_HELP is sent to the
		/// window that has the keyboard focus. If no window has the
		/// keyboard focus, WM_HELP is sent to the currently active window.
		/// </summary>
		HELP = 0x0053,

		/// <summary>
		/// The WM_USERCHANGED message is sent to all windows after the
		/// user has logged on or off. When the user logs on or off,
		...
		/// in the WM_NOTIFY notification message. WM_NOTIFYFORMAT
		/// messages are sent from a common control to its parent
		/// window and from the parent window to the common control.
		/// </summary>
		NOTIFYFORMAT = 0x0055,
		...
		/// SetWindowLong function is about to change one or more of
		/// the window's styles.
		/// </summary>
		...
		/// <summary>
		/// An application sends the WM_SETICON message to associate a
		/// new large or small icon with a window. The system displays
		/// the large icon in the ALT+TAB dialog box, and the small
		...
		/// free the allocated memory object associated with the
		/// window. The WM_NCDESTROY message is sent after the child
		...
		/// processing this message, an application can control the
		...
		/// released. If the mouse is not captured, the message is sent
		/// to the window beneath the cursor. Otherwise, the message is
		/// sent to the window that has captured the mouse.
		/// </summary>
		NCHITTEST = 0x0084,
		...
		/// nonclient area needs to be changed to indicate an active or
		/// inactive state.
		/// </summary>
		NCACTIVATE = 0x0086,
		...
		/// <summary>
		/// The WM_SYNCPAINT message is used to synchronize painting
		...
		/// cursor. If a window has captured the mouse, this message is
		/// not posted.
		/// </summary>
		NCMOUSEMOVE = 0x00A0,

		/// <summary>
		/// The WM_NCLBUTTONDOWN message is posted when the user
		...
		/// <summary>
		/// The WM_NCRBUTTONDOWN message is posted when the user
		/// presses the right mouse button while the cursor is within
		/// the nonclient area of a window. This message is posted to
		/// the window that contains the cursor. If a window has
		/// captured the mouse, this message is not posted.
		/// </summary>
		...
		/// double-clicks the middle mouse button while the cursor is
		/// within the nonclient area of a window. This message is
		/// posted to the window that contains the cursor. If a window
		/// has captured the mouse, this message is not posted.
		/// </summary>
		...
		/// presses the first or second X button while the cursor is in
		/// the nonclient area of a window. This message is posted to
		/// the window that contains the cursor. If a window has
		/// captured the mouse, this message is not posted.
		/// </summary>
		...
		/// <summary>
		/// The WM_NCXBUTTONUP message is posted when the user releases
		...
		/// <summary>
		/// The WM_INPUT_DEVICE_CHANGE message is sent to the window
		/// that registered to receive raw input. A window receives
		/// this message through its WindowProc function.
		/// </summary>
		INPUT_DEVICE_CHANGE = 0x00FE,

		/// <summary>
		/// The WM_INPUT message is sent to the window that is getting
		/// raw input.
		/// </summary>
		INPUT = 0x00FF,
		...
		/// <summary>
		/// The WM_DEADCHAR message is posted to the window with the
		/// keyboard focus when a WM_KEYUP message is translated by the
		/// TranslateMessage function. WM_DEADCHAR specifies a
		/// character code generated by a dead key. A dead key is a key
		/// that generates a character, such as the umlaut
		/// (double-dot), that is combined with another character to
		/// form a composite character. For example, the umlaut-O
		/// character (Ö) is generated by typing the dead key for the
		...
		/// presses another key. It also occurs when no window
		/// currently has the keyboard focus; in this case, the
		/// WM_SYSKEYDOWN message is sent to the active window. The
		/// window that receives the message can distinguish between
		...
		/// keyboard focus when the user releases a key that was
		/// pressed while the ALT key was held down. It also occurs
		/// when no window currently has the keyboard focus; in this
		/// case, the WM_SYSKEYUP message is sent to the active window.
		...
		/// by the TranslateMessage function. It specifies the
		...
		/// keyboard focus when a WM_SYSKEYDOWN message is translated
		/// by the TranslateMessage function. WM_SYSDEADCHAR specifies
		/// the character code of a system dead key — that is, a dead
		/// key that is pressed while holding down the ALT key.
		/// </summary>
		SYSDEADCHAR = 0x0107,

		/// <summary>
		/// The WM_UNICHAR message is posted to the window with the
		/// keyboard focus when a WM_KEYDOWN message is translated by
		/// the TranslateMessage function. The WM_UNICHAR message
		/// contains the character code of the key that was pressed.
		/// The WM_UNICHAR message is equivalent to WM_CHAR, but it
		/// uses Unicode Transformation Format (UTF)-32, whereas
		/// WM_CHAR uses UTF-16. It is designed to send or post Unicode
		/// characters to ANSI windows and it can can handle Unicode
		...
		/// string as a result of a keystroke. A window receives this
		...
		/// status as a result of a keystroke. A window receives this
		/// message through its WindowProc function.
		/// </summary>
		...
		/// command from the Window menu, clicks the maximize button,
		/// minimize button, restore button, close button, or moves the
		...
		/// event occurs in the window's standard horizontal scroll
		/// bar. This message is also sent to the owner of a horizontal
		...
		/// This message is also sent to the owner of a vertical scroll
		...
		/// <summary>
		/// The WM_ENTERIDLE message is sent to the owner window of a
		/// modal dialog box or menu that is entering an idle state. A
		/// modal dialog box or menu enters an idle state when no
		...
		/// <summary>
		/// An application sends the WM_QUERYUISTATE message to
		/// retrieve the user interface (UI) state for a window.
		/// </summary>
		QUERYUISTATE = 0x0129,

		/// <summary>
		/// The WM_CTLCOLORMSGBOX message is sent to the owner window
		/// of a message box before Windows draws the message box. By
		/// responding to this message, the owner window can set the
		/// text and background colors of the message box by using the
		...
		/// WM_CTLCOLOREDIT message to its parent window when the
		/// control is about to be drawn. By responding to this
		...
		/// parent window can set the text and background colors of the
		/// list box by using the specified display device context handle.
		/// </summary>
		...
		/// window of a scroll bar control when the control is about to
		/// be drawn. By responding to this message, the parent window
		/// can use the display context handle to set the background
		...
		/// <summary>
		/// Use WM_MOUSEFIRST to specify the first mouse message. Use
		...
		/// cursor moves. If the mouse is not captured, the message is
		/// posted to the window that contains the cursor. Otherwise,
		...
		/// <summary>
		/// The WM_MBUTTONDBLCLK message is posted when the user
		/// double-clicks the middle mouse button while the cursor is
		/// in the client area of a window. If the mouse is not
		...
		/// be no internal forwarding of the message, since
		/// DefWindowProc propagates it up the parent chain until it
		...
		/// <summary>
		/// The WM_MOUSEHWHEEL message is sent to the focus window when
		/// the mouse's horizontal scroll wheel is tilted or rotated.
		/// The DefWindowProc function propagates the message to the
		...
		/// <summary>
		/// Use WM_MOUSELAST to specify the last mouse message. Used
		/// with PeekMessage() Function.
		/// </summary>
		MOUSELAST = 0x020E,
		...
		/// resizing. By processing this message, an application can
		...
		/// multiple-document interface (MDI) client window to close an
		/// MDI child window.
		/// </summary>
		MDIDESTROY = 0x0221,
		...
		/// after it enters the moving or sizing modal loop. The window
		/// enters the moving or sizing modal loop when the user clicks
		/// the window's title bar or sizing border, or when the window
		/// passes the WM_SYSCOMMAND message to the DefWindowProc
		/// function and the wParam parameter of the message specifies
		/// the SC_MOVE or SC_SIZE value. The operation is complete
		/// when DefWindowProc returns. The system sends the
		/// WM_ENTERSIZEMOVE message regardless of whether the dragging
		/// of full windows is enabled.
		/// </summary>
		...
		/// window passes the WM_SYSCOMMAND message to the
		...
		/// window. A window receives this message through its
		/// WindowProc function.
		/// </summary>
		IME_NOTIFY = 0x0282,

		/// <summary>
		/// Sent by an application to direct the IME window to carry
		/// out the requested command. The application uses this
		/// message to control the IME window that it has created. To
		/// send this message, the application calls the SendMessage
		...
		/// to extend the area for the composition window. A window
		/// receives this message through its WindowProc function.
		/// </summary>
		...
		/// to change the current IME. A window receives this message
		...
		/// conversion result. A window receives this message through
		...
		/// <summary>
		/// Sent to an application by the IME to notify the application
		/// of a key press and to keep message order. A window receives
		/// this message through its WindowProc function.
		/// </summary>
		IME_KEYDOWN = 0x0290,
		...
		/// <summary>
		/// Sent to an application by the IME to notify the application
		/// of a key release and to keep message order. A window
		/// receives this message through its WindowProc function.
		/// </summary>
		IME_KEYUP = 0x0291,
		...
		/// cursor leaves the client area of the window specified in a
		...
		/// inserted only if the clipboard contains data in CF_TEXT format.
		/// </summary>
		PASTE = 0x0302,

		/// <summary>
		/// An application sends a WM_CLEAR message to an edit control
		/// or combo box to delete (clear) the current selection, if
		/// any, from the edit control.
		/// </summary>
		CLEAR = 0x0303,

		/// <summary>
		/// An application sends a WM_UNDO message to an edit control
		/// to undo the last operation. When this message is sent to an
		/// edit control, the previously deleted text is restored or
		...
		/// if it has delayed rendering a specific clipboard format and
		/// if an application has requested data in that format. The
		...
		/// <summary>
		/// The WM_DRAWCLIPBOARD message is sent to the first window in
		/// the clipboard viewer chain when the content of the
		...
		/// occurs in the clipboard viewer's vertical scroll bar. The
		/// owner should scroll the clipboard image and update the
		/// scroll bar values.
		/// </summary>
		...
		/// owner by a clipboard viewer window to request the name of a
		/// CF_OWNERDISPLAY clipboard format.
		/// </summary>
		ASKCBFORMATNAME = 0x030C,

		/// <summary>
		/// The WM_CHANGECBCHAIN message is sent to the first window in
		/// the clipboard viewer chain when a window is being removed
		/// from the chain.
		/// </summary>
		...
		/// the keyboard focus, giving the window the opportunity to
		...
		/// realizes its logical palette. This message enables windows
		/// that do not have the keyboard focus to realize their
		/// logical palettes and update their client areas.
		/// </summary>
		PALETTECHANGED = 0x0311,

		/// <summary>
		/// The WM_HOTKEY message is posted when the user presses a hot
		/// key registered by the RegisterHotKey function. The message
		/// is placed at the top of the message queue associated with
		/// the thread that registered the hot key.
		/// </summary>
		HOTKEY = 0x0312,

		/// <summary>
		/// The WM_PRINT message is sent to a window to request that it
		/// draw itself in the specified device context, most commonly
		...
		/// that it draw its client area in the specified device
		...
		/// clicking an application command button using the mouse or
		...
		/// events are the activation of a theme, the deactivation of a
		...
		/// rendering status of a window has changed. Only windows that
		/// have set the flag DWM_BLURBEHIND.fTransitionOnMaximized to
		/// true will get this message.
		/// </summary>
		DWMNCRENDERINGCHANGED = 0x031F,

		/// <summary>
		/// Sent to all top-level windows when the colorization color
		/// has changed.
		/// </summary>
		DWMCOLORIZATIONCOLORCHANGED = 0x0320,

		/// <summary>
		/// WM_DWMWINDOWMAXIMIZEDCHANGE will let you know when a DWM
		/// composed window is maximized. You also have to register for
		/// this message as well. You'd have other windowd go opaque
		/// when this message is sent.
		/// </summary>
		DWMWINDOWMAXIMIZEDCHANGE = 0x0321,

		/// <summary>
		/// Sent to request extended title bar information. A window
		/// receives this message through its WindowProc function.
		/// </summary>
		...
		/// <summary>
		/// An application sends the WM_CPL_LAUNCH message to Windows
		/// Control Panel to request that a Control Panel application
		/// be started.
		/// </summary>
		...
		/// application, started by the WM_CPL_LAUNCH message, has
		/// closed. The WM_CPL_LAUNCHED message is sent to the window
		/// identified by the wParam parameter of the WM_CPL_LAUNCH
		...
		/// <summary>
		/// The user completed an input event (for example, pressed an
		/// application command button on the mouse or an application
		/// command key on the keyboard), and the application did not
		/// handle the WM_APPCOMMAND message generated by that input.
		/// If the Shell procedure handles the WM_COMMAND message, it
		/// should not call CallNextHookEx. See the Return Value
		/// section for more information.
		/// </summary>
		HSHELL_APPCOMMAND = 12,

		/// <summary>
		/// A window is being minimized or maximized. The system needs
		/// the coordinates of the minimized rectangle for the window.
		/// </summary>
		HSHELL_GETMINRECT = 5,
		...
		/// <summary>
		/// The user has selected the task list. A shell application
		/// that provides a task list should return TRUE to prevent
		/// Windows from starting its task list.
		/// </summary>
		HSHELL_TASKMAN = 7,

		/// <summary>
		/// A top-level, unowned window has been created. The window
		/// exists when the system calls this hook.
		/// </summary>
		HSHELL_WINDOWCREATED = 1,

		/// <summary>
		/// A top-level, unowned window is about to be destroyed. The
		/// window still exists when the system calls this hook.
		/// </summary>
		HSHELL_WINDOWDESTROYED = 2,

		/// <summary>
		/// The activation has changed to a different top-level,
		/// unowned window.
		/// </summary>
		HSHELL_WINDOWACTIVATED = 4,

		/// <summary>
		/// A top-level window is being replaced. The window exists
	...
	public enum SystemCommand
	{
		...
		Maximize = 61488,
		NextWindow = 61504,
		PreviousWindow = 61520,
		Close = 61536,
		VerticalScroll = 61552,
		HorizontalScroll = 61568,
		MouseMenu = 61584,
		KeyMenu = 61696,
		Restore = 61728,
		Tasklist = 61744,
